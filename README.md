...
- 只接受对应航空公司的官方域名链接。
- 当前有效活动保存在 `data/campaigns.json`；方便人工查看的同步表格保存在 `data/campaigns.csv`。已明确过期的活动会直接清理，不继续保留。
- `data/best-deals.csv` 只列优惠力度较大的活动，例如至少 15% 折扣、立减 300 元以上、免费机票或免费航段权益，并按力度排序。
- CSV 使用带 BOM 的 UTF-8 编码，可直接用 Excel 打开中文内容；包含航司、标题、优惠等级、优惠亮点、关注度、出发地、目的地、购票期、旅行期、链接和摘要。
...
运行顺序：抓取 → 剔除明确过期活动 → 评估优惠力度 → 更新 JSON、完整 CSV 与精选优惠 CSV → 数据文件有变化才 commit/push → 有 NEW/UPDATED/EXPIRED 才创建 Issue。
